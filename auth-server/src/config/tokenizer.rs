...
pub fn generate_token(payload: &str, language_id: &str, ttl: i32) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let mut duration = std::time::Duration::from_secs(time_in_hours as u64);
    ...
    let mut token = local_paseto(&json_str, None, &mut &auth_token.into_bytes()).expect(&*t!("auth_failed", locale=&language_id));
    Ok(token)
}
